...
def search_products(
    ...
    优化：如果页面加载时已捕获足够数量的商品，直接使用，避免额外 API 请求。
    ...
    # 优化：检查页面加载时已捕获的商品是否满足数量要求
    captured = session.captured_products
    if captured and len(captured) >= limit:
        logger.info("页面已加载 %d 条商品，满足需求(%d条)，直接使用", len(captured), limit)
        return captured[:limit]
    elif captured:
        logger.info("页面已加载 %d 条商品，还需获取 %d 条", len(captured), limit - len(captured))

    all_products: list[Product] = list(captured) if captured else []
    # 如果已有捕获数据，从第2页开始请求（第1页已从页面加载获取）
    current_page = begin_page + (1 if captured else 0)
