...
class Browser:
    ...
    def get_existing_page(self, target_url_hint: str | None = None) -> Page | None:
        """获取已有页面。
        ...
        # 第一轮：优先匹配目标 URL（如 use_browser 预打开的 1688 页面）
        if target_url_hint:
            for target in targets:
                if target.get("type") == "page" and target_url_hint in target.get("url", ""):
                    target_id = target["id"]
                    try:
                        result = self._cdp.send(
                            "Target.attachToTarget",
                        ...
                        return self._setup_page(page)
                    except Exception:
                        continue

        # 第二轮：匹配任意非空白页面（兼容旧逻辑）
        for target in targets:
            if target.get("type") == "page" and target.get("url") not in (
                "about:blank",
                "chrome://newtab/",
            ):
                target_id = target["id"]
